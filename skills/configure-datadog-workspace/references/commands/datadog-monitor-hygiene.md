Produce the weekly **Datadog monitor hygiene report**: which monitors wake people up without reason, which ones stopped watching anything, and which gaps matter. The audience is the team lead who tunes alerting. The goal is fewer, better pages.
...
1. **Transition volume.** `aggregate_events`, `query: "source:alert"`, count grouped by `["@monitor_id","status"]`, `limit: 100`.
2. **Noisiest by day.** `aggregate_events` with the same query, count grouped by `["@monitor_id"]` with `group_by.interval: 86400000`, `limit: 20`. Shows whether noise is constant or comes in bursts.
...
4. **Stuck and silent.** `monitor_groups_search`, `query: "-group_status:ok"`, `per_page: 50`. Use `counts` for the No Data total and keep groups whose status is `No Data`. Then `search_datadog_monitors`, `query: "muted:true"`.
...
7. **SLOs at risk.** `search_datadog_slos`.

## Classify each monitor you report as exactly one of

- **Flapping**: ≥ 10 alert/recover pairs in 7 days. Suggest a longer evaluation window, a recovery threshold, or a `last_Xm` change.
- **Always on**: alerting most of the week with no recovery. Either a real unfixed problem (escalate) or a wrong threshold (retune).
...
**Summary:** <N> transitions this week · <n> monitors cause <x%> of all transitions · <n> silent · <n> muted forever.
